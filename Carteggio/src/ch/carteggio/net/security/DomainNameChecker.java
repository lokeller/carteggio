...
public class DomainNameChecker {
    ...
    private static boolean isIpAddress(String domain) {
        ...
        try {
            ...
            if (DEBUG) {
                Log.v(LOG_TAG, "DomainNameChecker.isIpAddress(): "
                      + errorMessage);
            }
    ...
    private static boolean matchIpAddress(X509Certificate certificate, String thisDomain) {
        if (DEBUG) {
            Log.v(LOG_TAG, "DomainNameChecker.matchIpAddress(): this domain: " + thisDomain);
        }
                ...
                for (List<?> altNameEntry : subjectAltNames) {
                    if ((altNameEntry != null) && (2 <= altNameEntry.size())) {
                        ...
                        if (altNameType != null && altNameType.intValue() == ALT_IPA_NAME) {
                            ...
                            if (altName != null) {
                                if (DEBUG) {
                                    Log.v(LOG_TAG, "alternative IP: " + altName);
                                }
    ...
    private static boolean matchDns(X509Certificate certificate, String thisDomain) {
        ...
        try {
            ...
            if (DEBUG) {
                ...
                Log.v(LOG_TAG, "DomainNameChecker.matchDns(): "
                      + errorMessage);
            }
        }
